...
serve(async (req) => {
  ...
  try {
    ...
    const body: XenditPaymentRequest = await req.json();
    ...
    console.log('[create-xendit-payment] Checking booking:', { bookingId, bookingIdInt, userId });
    ...
    console.log('[create-xendit-payment] Booking query result:', { booking, bookingError });
    ...
    if (paymentMethod === 'invoice') {
      ...
      const invoiceResponse = await fetch(`${XENDIT_BASE_URL}/v2/invoices`, {
        ...
        body: JSON.stringify({
          ...
          payment_methods: ['GCASH', 'PAYMAYA', 'BPI', 'BDO', 'UNIONBANK', 'DRAGONPAY'],
          metadata: {
      ...
      if (!ewalletResponse.ok) {
        ...
        return new Response(
          JSON.stringify({ error: 'Failed to create eWallet charge', details: errorData }),
          { status: ewalletResponse.status, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
...
function getChannelCode(paymentMethod: string): string {
